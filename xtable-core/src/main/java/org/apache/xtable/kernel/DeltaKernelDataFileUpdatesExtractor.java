...
public class DeltaKernelDataFileUpdatesExtractor {
    @Builder.Default
    private final DeltaKernelStatsExtractor deltaStatsExtractor = DeltaKernelStatsExtractor.getInstance();

    @Builder.Default
    private final DeltaKernelPartitionExtractor deltaKernelPartitionExtractor = DeltaKernelPartitionExtractor.getInstance();


    @Builder.Default
    private final DeltaKernelDataFileExtractor dataFileExtractor = DeltaKernelDataFileExtractor.builder().build();
    ...
    public Seq<RowBackedAction> applySnapshot(
        ...
        if (tableExists) {
            ...
            try {
                ...
                System.err.println("WARNING: Delta Kernel 4.0.0 bug - NullPointerException reading snapshot without checkpoint");
                System.err.println("This is a known issue with tables that have < 10 commits");
                System.err.println("File removals will not be detected until first checkpoint is created (at 10th commit)");
                snapshotReadFailed = true;
            } catch (Exception e) {
                System.err.println("ERROR: Failed to get snapshot: " + e.getClass().getName() + ": " + e.getMessage());
                e.printStackTrace();
                throw e;
            }
            ...
            if (snapshotReadFailed) {
                ...
                CloseableIterator<FilteredColumnarBatch> scanFiles = myScan.getScanFiles(engine, includeColumnStats);
                ...
                System.out.println("Found " + fileCount + " existing files in Delta table (from " + batchCount + " batches)");
        ...
        System.out.println("ApplySnapshot diff: " +
                          diff.getFilesAdded().size() + " files to add, " +
                          diff.getFilesRemoved().size() + " files to remove");
    ...
    private boolean checkTableExists(String tablePath) {
        ...
        public Seq<RowBackedAction> applyDiff (
                InternalFilesDiff internalFilesDiff, InternalSchema tableSchema, String tableBasePath, StructType physicalSchema){
            List<RowBackedAction> removeActions =
                    internalFilesDiff.dataFilesRemoved().stream()
                            .flatMap(dFile -> createAddFileAction(dFile, tableSchema, tableBasePath, physicalSchema))
            ...
            return applyDiff(internalFilesDiff.dataFilesAdded(), removeActions, tableSchema, tableBasePath, physicalSchema);
        }
        ...
        private Stream<AddFile> createAddFileAction (
                InternalDataFile dataFile, InternalSchema schema, String tableBasePath, StructType physicalSchema){
        ...
        private String getColumnStats (
                InternalSchema schema,long recordCount, List<org.apache.xtable.model.stat.ColumnStat > columnStats){
            try {
