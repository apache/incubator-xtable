...
public class DeltaKernelConversionTarget implements ConversionTarget {
    ...
    public void beginSync(InternalTable table) {
        this.transactionState =
                new DeltaKernelConversionTarget.TransactionState(engine, tableName, table.getLatestCommitTime(), logRetentionInHours);
           }
    @Override
    ...
    public void syncFilesForSnapshot(List<PartitionFileGroup> partitionedDataFiles) {
        ...
        transactionState.setActions(
                dataKernelFileUpdatesExtractor.applySnapshot(table, partitionedDataFiles, transactionState.getLatestSchemaInternal()));
    }
    ...
    public void syncFilesForDiff(InternalFilesDiff internalFilesDiff) {
        ...
        transactionState.setActions(
                dataKernelFileUpdatesExtractor.applyDiff(
                        ...
                        table.getPath(engine).toString(),table.getLatestSnapshot(engine).getSchema()));
    }
    ...
    public Optional<String> getTargetCommitIdentifier(String sourceIdentifier) {
        ...
        io.delta.kernel.internal.TableImpl tableImpl =
            (io.delta.kernel.internal.TableImpl) table;
        ...
        try (io.delta.kernel.utils.CloseableIterator<io.delta.kernel.data.ColumnarBatch> iter =
            ...
            while (iter.hasNext()) {
                ...
                int commitInfoIndex = batch.getSchema().indexOf(
                        ...
                        for (int i = 0; i < tagSize; i++) {
                            ...
                            if (TableSyncMetadata.XTABLE_METADATA.equals(key)) {
                                ...
                                try {
                                    ...
                                    System.err.println("Failed to parse commit metadata for version " +
    ...
    private class TransactionState {
        ...
        private void commitTransaction() {
            ...
            if (tableExists) {
                ...
                if (schemaChanged) {
                    ...
                    io.delta.kernel.data.Row metadataRow =
                        io.delta.kernel.internal.actions.SingleAction.createMetadataSingleAction(newMetadata.toRow());
            ...
            while (actionsIterator.hasNext()) {
                ...
                if (action instanceof io.delta.kernel.internal.actions.AddFile) {
                    ...
                    io.delta.kernel.data.Row wrappedRow =
                        io.delta.kernel.internal.actions.SingleAction.createAddFileSingleAction(addFile.toRow());
                    ...
                    io.delta.kernel.data.Row wrappedRow =
                        io.delta.kernel.internal.actions.SingleAction.createRemoveFileSingleAction(removeFile.toRow());
                    allActionRows.add(wrappedRow);
                }
            }
            ...
            try {
                ...
                if (hooks != null && !hooks.isEmpty()) {
                    ...
                    for (io.delta.kernel.hook.PostCommitHook hook : hooks) {
                        ...
                        try {
                            ...
                            if (hook.getType() == io.delta.kernel.hook.PostCommitHook.PostCommitHookType.CHECKPOINT) {
                                System.out.println("Checkpoint created via PostCommitHook at version " + result.getVersion());
                            }
                            ...
                            System.err.println("Warning: PostCommitHook failed with unexpected exception: " + hookEx.getMessage());
                            hookEx.printStackTrace();
                        }
                    }
                } else {
                ...
                e.printStackTrace();
                throw new RuntimeException("Failed to commit Delta Kernel transaction: " + e.getMessage(), e);
            }
            ...
            // - Operation type metadata (like DeltaOperations.Update) is simplified to Operation.WRITE/CREATE_TABLE
        ...
        private Map<String, String> getConfigurationsForDeltaSync(boolean tableExists) {
            ...
            configMap.put(
                ...
                String.format("interval %d hours", retentionInHours));
        ...
        private String getFileFormat() {
            ...
            Table table = Table.forPath(engine, basePath);
            io.delta.kernel.Snapshot snapshot = table.getLatestSnapshot(engine);
            io.delta.kernel.internal.SnapshotImpl snapshotImpl = (io.delta.kernel.internal.SnapshotImpl) snapshot;
