...
public class ParquetPartitionValueExtractor {
    ...
    for (int i = 0; i < partitions.size(); i++) {
      ...
      switch (partitionField.getTransformType()) {
        case YEAR:
          value =
              EPOCH
                  .plusYears(Long.parseLong(partitionField.getPartitionValue()))
                  .toInstant()
                  .toEpochMilli();
          break;
        case MONTH:
          value =
              EPOCH
                  .plusMonths(Long.parseLong(partitionField.getPartitionValue()))
                  .toInstant()
                  .toEpochMilli();
          break;
        case DAY:
          value =
              EPOCH
                  .plusDays(Long.parseLong(partitionField.getPartitionValue()))
                  .toInstant()
                  .toEpochMilli();
          break;
        case HOUR:
          value =
              EPOCH
                  .plusHours(Long.parseLong(partitionField.getPartitionValue()))
                  .toInstant()
                  .toEpochMilli();
          break;
        default:
  ...
  public List<InternalPartitionField> convertFromParquertUserDefinedPartitionConfig(
      InputPartitionFields userDefinedPartitionSchema) {
  ...
  public List<PartitionValue> createPartitionValues(InputPartitionFields partitionsConf) {
    Map<InternalPartitionField, Range> extractedPartitions = extractPartitionValues(partitionsConf);
    return extractedPartitions.entrySet().stream()
        .map(
            internalPartitionField ->
                PartitionValue.builder()
                    .partitionField(internalPartitionField.getKey())
