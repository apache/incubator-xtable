...
/**
 * Shared utility for Delta stats conversion between XTable internal format and Delta format. Used
 * by both Delta Standalone and Delta Kernel implementations.
 */
@Log4j2
public class DeltaStatsUtils {
  ...
  /**
   * Converts XTable internal column statistics to Delta format JSON string.
   *
   * @param schema the table schema
   * @param numRecords the number of records in the file
   * @param columnStats the list of column statistics
   * @return JSON string in Delta stats format
   * @throws JsonProcessingException if serialization fails
   */
  public static String convertStatsToDeltaFormat(
  ...
  /**
   * Parses Delta stats JSON and extracts column statistics for the specified fields.
   *
   * @param statsJson the Delta stats JSON string
   * @param fields the fields to extract stats for
   * @return FileStats containing column statistics and record count
   * @throws ParseException if JSON parsing fails
   */
  public static FileStats parseColumnStatsFromJson(String statsJson, List<InternalField> fields) {
    if (StringUtils.isEmpty(statsJson)) {
      return FileStats.builder().columnStats(Collections.emptyList()).numRecords(0).build();
    }

    try {
      DeltaStats deltaStats = MAPPER.readValue(statsJson, DeltaStats.class);
  ...
  private static Set<String> getPathsFromStructSchemaForMinAndMaxStats(InternalSchema schema) {
  ...
  private static Map<String, Object> getMinValues(List<ColumnStat> validColumnStats) {
    return getValues(validColumnStats, columnStat -> columnStat.getRange().getMinValue());
  }

  private static Map<String, Object> getMaxValues(List<ColumnStat> validColumnStats) {
  ...
  private static Map<String, Object> getNullCount(List<ColumnStat> validColumnStats) {
  ...
  private static void insertValueAtPath(
      Map<String, Object> jsonObject, String[] pathParts, Object value) {
  ...
  private static void collectUnsupportedStats(Map<String, Object> additionalStats) {
   ...
   */
  private static Map<String, Object> flattenStatMap(Map<String, Object> statMap) {
  ...
  /** Helper class for flattening nested stat maps. */
  @Value
  @AllArgsConstructor(staticName = "of")
  static class StatField {
    String parentPath;
