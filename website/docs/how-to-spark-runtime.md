...
1. **Inside a Spark job you already run**, by calling `XTableSyncService` right after your write,
   so a table stays interoperable without a separate process or cluster.
2. **As its own job**, with the `XTableSparkSync` entry point, the equivalent of `RunSync` for
   this bundle.
...
table to be interoperable with the others as soon as the write finishes, without a second submit
...
`partitionSpec` carries the source partitioning into the targets, and is the same setting as the
`--partitionspec` flag that [Running a sync as its own job](#running-a-sync-as-its-own-job) takes.
Leave it out for an unpartitioned source. Leaving it out for a partitioned one creates the targets
as unpartitioned.
...
The jar also ships a `spark-submit` entry point, `org.apache.xtable.spark.XTableSparkSync`, for
syncing a table you did not write in this job. Pass the jar as the application jar rather than with
`--jars`. It runs in one of two modes.
