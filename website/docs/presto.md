...
over other columns in the Delta table. During sync, XTable uses the same logic to generate partition columns wherever required. 
Currently, the generated columns from XTable sync shows `NULL` when queried from Presto CLI.
:::

For hands on experimentation, please follow [Creating your first interoperable table](/docs/how-to) tutorial
to create XTable synced tables followed by [Hive Metastore](/docs/hms) tutorial to register the target table
