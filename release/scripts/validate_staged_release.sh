...
FULL_SVN_URL=${ROOT_SVN_URL}/${REPO_TYPE}/${XTABLE_REPO}/${ARTIFACT_PREFIX}${ARTIFACT_SUFFIX}
...
echo "Validating xtable-${ARTIFACT_SUFFIX} with release type \"${REPO_TYPE}\""
...
(bash -c "gpg --import ../KEYS $REDIRECT" && bash -c "gpg --verify xtable-${ARTIFACT_SUFFIX}.src.tgz.asc xtable-${ARTIFACT_SUFFIX}.src.tgz $REDIRECT" && echo -e "\t\tSignature Check - [OK]\n") || (echo -e "\t\tSignature Check - [ERROR]\n\t\t Run with --verbose to get details\n" && exit 1)

# Untar
(bash -c "tar -zxf xtable-${ARTIFACT_SUFFIX}.src.tgz $REDIRECT") || (echo -e "\t\t Unable to untar xtable-${ARTIFACT_SUFFIX}.src.tgz - [ERROR]\n\t\t Please run with --verbose to get details\n" && exit 1)
cd xtable-${ARTIFACT_SUFFIX}
