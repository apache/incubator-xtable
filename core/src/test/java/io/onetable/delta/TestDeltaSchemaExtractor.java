  ...
  void generateColumnsAreNotTranslatedToInternalSchema() {
    StructType structRepresentation =
        new StructType()
            ...
            .add("birthYear", DataTypes.TimestampType, true, Metadata.fromJson("{\"delta.generationExpression\":\"YEAR(birthDate)\"}"));
    OneSchema oneSchemaRepresentation = OneSchema.builder()
        .dataType(OneType.RECORD)
        .name("struct")
        .fields(Collections.singletonList(
            OneField.builder()
                .schema(OneSchema.builder().name("timestamp").dataType(OneType.TIMESTAMP).metadata(Collections.singletonMap(OneSchema.MetadataKey.TIMESTAMP_PRECISION, OneSchema.MetadataValue.MICROS)).build())
                .name("birthDate")
                .build()))
        .build();
    Assertions.assertEquals(
