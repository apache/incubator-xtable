...
public class IcebergSourceClient implements SourceClient<Long> {
  ...
  public TableChange getTableChangeForCommit(Long snapshotId) {
    ...
    OneTable table = getTable(snapshotId);
  ...
  public CommitsBacklog<Long> getCommitsBacklog(InstantsForIncrementalSync lastSyncInstant) {
    ...
    while (pendingSnapshot != null && pendingSnapshot.timestampMillis() > epochMilli) {
      snapshots.add(pendingSnapshot.snapshotId());
      pendingSnapshot =
    ...
    return CommitsBacklog.<Long>builder().commitsToProcess(snapshots).build();
  }
  ...
  public boolean isIncrementalSyncSafeFrom(Instant instant) {
    ...
    Table iceTable = getSourceTable();
    Iterable<Snapshot> iterable = iceTable.snapshots();
    ...
    boolean doesChainBreaks = false;
    // Checking if chain breaks in traversal order of snapshots and snapshots are deleted can
    // help with this.
    return !doesChainBreaks;
  }
}
